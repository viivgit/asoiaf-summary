...
#from clean import condense, condense2 # ADDED BY ME but useless in the end
...
class _Mimetype(object):
    ...
    def __init__(self, parent_directory):
        mimetype_template = os.path.join(EPUB_TEMPLATES_DIR, 'mimetype.txt')
...
class _EpubFile(object):
    ...
    def write(self, file_name):
        ...
        with open(file_name, 'wb') as f:
            f.write(self.content.encode('utf-8')) # CHANGED BY ME (added condense() but then removed it, so it's unchanged)
...
class TocHtml(_EpubFile):
    ...
    def add_chapters(self, chapter_list):
        ...
        file_name_list = [c.file_name for c in chapter_list] # added by me
#        link_list = [str(n) + '.xhtml' for n in chapter_numbers]
        link_list = [file_name_list[n] for n in chapter_numbers] # added by me
        try:
...
#        link_list = [str(n) + '.xhtml' for n in id_list]
        link_list = [file_name_list[n] for n in id_list] # added by me
...
#        chapter_file_output = os.path.join(self.OEBPS_DIR, self.current_chapter_path)
        chapter_file_output = os.path.join(self.OEBPS_DIR, c.file_name)
#        c._replace_images_in_chapter(self.OEBPS_DIR) # commented by me
