...
import clean
from clean import condense, condense2
...
class Chapter(object):
    ...
    Attributes:
        ...
        html_title (str): Title string with special characters replaced with
            html-safe sequences
    """
    def __init__(self, content, title, file_name, url=None):
...
#            assert file_name[-6:] == '.xhtml'
            assert file_name[-5:] == '.html' # added by me
        except (AssertionError, IndexError):
            raise ValueError('filename must end with .html') # changed by me from xhtml
        with open(file_name, 'wb') as f:
            f.write(self.content.encode('utf-8')) # CHANGED BY ME (added condense())
...
class ChapterFactory(object):
    ...
    def create_chapter_from_file(self, file_name, url=None, title=None):
        ...
        return self.create_chapter_from_string(content_string, file_name, url, title)

    def create_chapter_from_string(self, html_string, file_name, url=None, title=None):
        """
        ...
        clean_xhtml_string = clean.html_to_xhtml2(clean_html_string)
#        clean_xhtml_string = clean.condense(clean_xhtml_string) # ADDED BY ME
        ...
        if title:
            ...
            try:
                ...
                title = 'Ebook Chapter'
        return Chapter(clean_xhtml_string, title, file_name, url)
